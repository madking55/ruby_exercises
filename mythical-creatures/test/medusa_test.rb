...
class MedusaTest < Minitest::Test
  ...
  def test_can_only_have_three_victims
    medusa = Medusa.new("Cassiopeia")
    medusa.stare(Person.new("Bolek"))
    medusa.stare(Person.new("Lolek"))
    medusa.stare(Person.new("Tola"))

    assert_equal 3, medusa.statues.count

    medusa.stare(Person.new("Reksio"))
    assert_equal 3, medusa.statues.count
  end
  ...
  def test_if_a_fourth_victim_is_stoned_first_is_unstoned
    ...
    medusa.stare(Person.new("Tola"))
    medusa.stare(Person.new("Reksio"))
    assert_equal "Reksio",  medusa.statues.last.name
    assert  medusa.statues.last.stoned?
    
    refute medusa.statues.first.name == "Bolek"
  end
